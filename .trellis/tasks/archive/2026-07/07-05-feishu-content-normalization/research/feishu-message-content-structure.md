...
- 来源：飞书开放平台文档“接收消息内容结构”。
- 用途：作为本任务实现 Feishu/Lark `message.content` 归一化的结构依据。
- 快照文件：`research/feishu-message-content-structure.md`。
- 说明：这是任务内调研快照。实现和测试以该快照列出的消息类型、字段和示例结构为基准；如官方文档后续变更，应另行更新本快照并同步调整 PRD/设计/测试。
...
- **默认情况**：接收到的卡片结构与实际发送时的卡片 JSON 结构不一致，具体格式请参考下方的 JSON 示例：
- 部分接口支持通过传入参数来获取原始卡片 JSON，详情请参考对应接口中 **`card_msg_content_type`** 参数的说明：
  1. [获取会话历史消息](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/list)
  2. [获取指定消息的内容](https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/get)
