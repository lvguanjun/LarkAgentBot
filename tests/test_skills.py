...
def test_system_prompt_fragment_lists_only_tier_one_metadata(tmp_path: Path) -> None:
    ...
    fragment = SkillsRegistry.discover(defaults, tmp_path / "groups" / "chat-1").get_system_prompt_fragment()
...
async def test_builtin_tool_reads_skill_and_reference_file(tmp_path: Path) -> None:
    ...
    reference_text = await tools.call_tool("read_skill", {"name": "writer", "file": "references/style.md"})
...
async def test_read_skill_rejects_unsafe_reference_paths(tmp_path: Path) -> None:
    ...
    assert "Error:" in await tools.call_tool("read_skill", {"name": "writer", "file": "/tmp/secret.md"})
    assert "Error:" in await tools.call_tool("read_skill", {"name": "writer", "file": "references/../SKILL.md"})
    assert "Error:" in await tools.call_tool("read_skill", {"name": "writer", "file": "SKILL.md"})
    assert "Error:" in await tools.call_tool("read_skill", {"name": "writer", "file": "references/escape.md"})
